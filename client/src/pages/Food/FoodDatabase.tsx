...
const DUMMY_FOODS = [
  ...
  {
    food_id: "2",
    food_name: "Spicy Basil Pork with Rice",
    price: 50,
...
const FoodDatabase = () => {
  ...
  const displayedFoods = useMemo(() => {
    ...
    if (macroTarget.amount !== "") {
      ...
      result.sort((a, b) => {
        const aVal =
          a.macronutrient[macroTarget.type as keyof typeof a.macronutrient];
        const bVal =
          b.macronutrient[macroTarget.type as keyof typeof b.macronutrient];
  ...
  const uniqueCanteens = [
    "All",
    ...Array.from(new Set(DUMMY_FOODS.map((f) => f.canteen))),
  ];
  ...
  return (
    <div className="min-h-screen bg-stone-50 flex font-sans">
      ...
      <div className="hidden md:flex flex-col w-1/3 max-w-sm bg-stone-900 text-white p-10 relative border-r border-stone-800">
        ...
        <Link
          to="/"
          className="text-stone-400 hover:text-white flex items-center gap-2 transition w-fit group"
        >
          <svg
        ...
        <div className="mt-32">
          <h1 className="font-serif text-5xl text-amber-400 mb-6 leading-tight">
            Food
            <br />
            Database
          </h1>
          <p className="text-stone-400 leading-relaxed text-sm">
            Search, filter, and discover meals from your campus canteens to
            perfectly hit your macro targets.
          </p>
        </div>
      ...
      <div className="flex-1 flex flex-col h-screen overflow-hidden relative">
        ...
        <div className="md:hidden bg-stone-900 px-6 py-6 text-white flex items-center justify-between shadow-md z-10">
          ...
          <h1 className="text-xl font-serif tracking-wide text-amber-400">
            Food Database
          </h1>
          <div className="w-6" />
        </div>
        ...
        <div className="p-5 md:p-8 flex-1 overflow-y-auto">
          ...
          <div className="flex gap-3 overflow-x-auto pb-4 scrollbar-hide -mx-5 px-5 md:mx-0 md:px-0">
            ...
            <button
              onClick={() =>
                setActiveFilterModal(
                  activeFilterModal === "macro" ? null : "macro",
                )
              }
              className={`flex-shrink-0 flex items-center gap-2 px-5 py-2.5 rounded-full text-xs font-bold uppercase tracking-widest transition shadow-sm border ${activeFilterModal === "macro" || macroTarget.amount !== "" ? "bg-amber-50 border-amber-400 text-amber-600" : "bg-white border-stone-200 text-stone-600 hover:border-amber-400 hover:text-amber-500"}`}
            >
              Macros
            </button>
            ...
            <button
              onClick={() =>
                setActiveFilterModal(
                  activeFilterModal === "price" ? null : "price",
                )
              }
              className={`flex-shrink-0 flex items-center gap-2 px-5 py-2.5 rounded-full text-xs font-bold uppercase tracking-widest transition shadow-sm border ${activeFilterModal === "price" || priceTarget.amount !== "" ? "bg-amber-50 border-amber-400 text-amber-600" : "bg-white border-stone-200 text-stone-600 hover:border-amber-400 hover:text-amber-500"}`}
            >
              Price
            </button>
            ...
            <button
              onClick={() =>
                setActiveFilterModal(
                  activeFilterModal === "canteen" ? null : "canteen",
                )
              }
              className={`flex-shrink-0 flex items-center gap-2 px-5 py-2.5 rounded-full text-xs font-bold uppercase tracking-widest transition shadow-sm border ${activeFilterModal === "canteen" || canteenFilter !== "All" ? "bg-amber-50 border-amber-400 text-amber-600" : "bg-white border-stone-200 text-stone-600 hover:border-amber-400 hover:text-amber-500"}`}
            >
              Canteen
            </button>
          ...
          {activeFilterModal === "macro" && (
            <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 mb-6 animate-fade-in">
              <div className="flex justify-between items-center mb-3">
                ...
                <button
                  onClick={() => {
                    setMacroTarget({ type: "protein", amount: "" });
                    setActiveFilterModal(null);
                  }}
                  className="text-[10px] text-amber-700 hover:underline uppercase font-bold"
                >
                  Clear
                </button>
              </div>
              <div className="flex gap-2">
                <select
                  value={macroTarget.type}
                  onChange={(e) =>
                    setMacroTarget({ ...macroTarget, type: e.target.value })
                  }
                ...
                <input
                  type="number"
                  placeholder="Target"
                  value={macroTarget.amount}
                  onChange={(e) =>
                    setMacroTarget({ ...macroTarget, amount: e.target.value })
                  }
          ...
          {activeFilterModal === "price" && (
            <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 mb-6 animate-fade-in">
              <div className="flex justify-between items-center mb-3">
                ...
                <button
                  onClick={() => {
                    setPriceTarget({ mode: "", sort: "", amount: "" });
                    setActiveFilterModal(null);
                  }}
                  className="text-[10px] text-amber-700 hover:underline uppercase font-bold"
                >
              ...
              <div className="flex flex-col gap-3">
                <div className="flex gap-2">
                  <button
                    onClick={() =>
                      setPriceTarget({ mode: "sort", sort: "asc", amount: "" })
                    }
                  ...
                  <button
                    onClick={() =>
                      setPriceTarget({ mode: "sort", sort: "desc", amount: "" })
                    }
                ...
                <div className="flex items-center gap-2">
                  ...
                  <input
                    ...
                    placeholder="40"
                    value={
                      priceTarget.mode === "closest" ? priceTarget.amount : ""
                    }
                    onChange={(e) =>
                      setPriceTarget({
                        mode: "closest",
            ...
            {displayedFoods.length > 0 ? (
              displayedFoods.map((food) => (
                <Link
                  ...
                  key={food.food_id}
                  className="bg-white border border-stone-100 rounded-3xl p-3 shadow-sm hover:shadow-md transition group cursor-pointer flex flex-col block"
                >
                  {/* Image Placeholder */}
                  ...
                  <div className="px-1 flex-1 flex flex-col">
                    ...
                    <h3 className="font-bold text-stone-800 mb-3 line-clamp-1">
                      {food.food_name}
                    </h3>
                    ...
                    <div className="mt-auto flex justify-between items-center pt-3 border-t border-stone-100">
                      ...
                      <div className="flex gap-1.5 text-[9px] text-stone-500 font-medium">
                        <span title="Protein">
                          Protein:{food.macronutrient.protein}
                        </span>
                        <span title="Carbs">
                          Carb:{food.macronutrient.carbohydrate}
                        </span>
                        <span title="Fat">Fat:{food.macronutrient.fat}</span>
                        <span title="Sugar">
                          Sugar:{food.macronutrient.sugar}
                        </span>
                      </div>
                    </div>
                  </div>
