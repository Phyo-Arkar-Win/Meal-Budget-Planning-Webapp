...
const AddFood = () => {
  ...
  const isExactMatch = canteenList.some(
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!selectedCanteen) {
      setError("Please select a canteen location.");
      return;
    }
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-stone-900/60 backdrop-blur-sm p-6">
          <div className="bg-white w-full max-w-sm rounded-3xl p-6 shadow-2xl">
            ...
            <p className="text-stone-500 text-center text-sm mb-6">
              ...
              <span className="font-bold text-stone-900">"{pendingCanteenName}"</span>{" "}
      ...
      <main className="max-w-2xl mx-auto p-6 mt-4">
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="bg-white rounded-3xl p-6 border border-stone-200 shadow-sm relative overflow-hidden">
            ...
            <div className="mb-6">
              <label className="block text-[10px] font-bold text-amber-500 uppercase tracking-widest mb-2 text-center">
                Total Calories (kcal)
              </label>
          ...
          <button type="submit" disabled={submitting}
            className="w-full bg-stone-900 text-white p-5 rounded-2xl font-bold uppercase tracking-widest hover:bg-amber-400 hover:text-stone-900 transition shadow-lg hover:shadow-amber-400/20 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-3">
            {submitting ? (
              <>
