...
export const createFood = async (req: Request, res: Response) => {
  try {
    ...
    // macros comes in as a JSON string when sent via FormData
    let macros: any;
    try {
      macros = typeof req.body.macros === "string"
        ? JSON.parse(req.body.macros)
        : req.body.macros;
    } catch {
      return res.status(400).json({ message: "Invalid macros format." });
    }

    if (!name || !price || !canteen || !macros) {
      return res.status(400).json({ message: "name, price, canteen and macros are required." });
    }

    // req.file is set by multer-storage-cloudinary — .path is the Cloudinary URL
    const picture = (req.file as any)?.path ?? "";

    const food = await Food.create({ name, price: Number(price), canteen, picture, macros });
...
export const seedFoods = async (req: Request, res: Response) => {
  try {
    ...
    res.status(201).json({ message: `${samples.length} foods seeded successfully!`, total: all.length, data: all });
  } catch (error: any) {
    res.status(500).json({ message: "Server Error", error: error.message });
  }
};